...
“自动创建设备平台”开关已移除。即使设置关闭普通写入确认，脚本源码确认仍不可跳过。
...
0.7.1 已在本机 Revit 2026.5 的 Autodesk 自带 Structural Analysis 模板中完成宿主烟雾测试：连续上下文读取令牌稳定；C#查询脚本由真实 Revit 编译、审阅并返回2个标高和空轴网/柱梁统计；写脚本以“常规-200mm”在标高1试建6000×4000×3000mm四面墙并回滚，回读确认墙数仍为0；正式提交后生成4面墙，ElementId逐项回读的墙型、标高、长度、高度及注释正确；同一脚本ID不能重复提交。测试模板未保存，不涉及用户正式项目。

便携测试项目可编译；本机策略阻止测试程序运行，因此便携用例在GitHub CI验证。便携测试使用最小Document替身。真实宿主测试确认脚本执行引擎，不等于所有生成脚本、族、项目或AI模型均已验收。外部测试进程无法调用加密的AI供应商配置，最终自然语言规划仍需在Revit聊天面板内验收。
