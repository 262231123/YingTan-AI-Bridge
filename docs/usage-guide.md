...
0.7.9 起，`K-H×16-17`、`K-H轴×16-17轴` 等乘号分隔的轴网表达可以直接预取模型范围。AI 响应若在 JSON 中途截断，宿主会要求其重新返回简短、完整的计划。设备平台参数失败会显示具体字段；`foundationOffsetMm=0` 是合法值，而 `equipmentTopMm` 必须大于 `sideTopMm`，以避免两组梁在同一高度重叠。

0.7.8 起，钢梁截面高度同时支持 Revit 标准参数和 `d`/`h`/截面高度等常用族类型参数，类型查询会返回 `depthParameter` 说明实际数据来源。AI 说“将生成/编译/执行脚本”却没有提供真实命令，或返回空内容时，宿主会自动要求其补全可执行计划。

0.7.7 起默认启用“全局直接建模”。所有对话产生的墙、门窗、房间、钢平台、图纸和其他写入计划，都会在必要查询及内部可回滚试运行通过后自动提交，不显示黄色“待执行计划”。如需先审阅，可在 `配置 → Agent` 关闭“全局直接建模”。任意 C# 脚本仍必须完成源码审阅，该安全边界不可关闭。
...
0.7.3 起，如果 AI 在文字中声称“现在/接下来执行查询或预览”，却错误返回空计划，宿主会自动要求它补交实际 JSON 命令并在同一轮继续。只有真正缺少设计条件时才会停下来询问。

0.7.2 起，聊天面板底部“成功/失败”状态可点击。处理记录窗口显示本次 Revit 会话最近 200 条命令的时间、模型、状态、耗时、返回 JSON 和错误原因，并可打开完整 `bridge.log`。设备平台自动执行选项已移除，写入统一从待执行计划确认。

0.7.0 新增[对话脚本补充查询与操作](live-scripting.md)。脚本运行需要源码审阅；Dynamo/pyRevit仍仅生成工件。

0.6.0 新增[轴网定位与设备钢平台](steel-platform.md)，支持补齐条件后自动创建概念平台。发布 EXE 的 Revit 组件针对 Revit 2026.5（.NET 10）构建；本文后续 2027 路径为源码默认示例。
