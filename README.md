...
0.7.1 预览版新增并实测[对话脚本执行](docs/live-scripting.md)：原生命令不足时主动编译 C# 查询/操作脚本，审阅查询结果后继续规划，写入先试运行回滚再确认提交。修复 Revit 托管包装器变化导致同一文档令牌漂移的问题。不是隔离沙箱，所有脚本运行均强制审阅源码，不受平台自动执行开关豁免。

0.6.0 预览版新增[轴网定位与设备钢平台](docs/steel-platform.md)：读取当前/链接模型轴网 → 补齐设计条件 → 比较分跨方案 → 真实族试建并回滚检查 → 确认后创建钢柱、梁和三块平台板。可勾选本次对话的平台自动执行模式。
