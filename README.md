...
0.7.9 预览版修复设备平台计划无回复和误判参数的问题：支持 `K-H×16-17`/`K-H轴×16-17轴` 轴网写法；截断或残缺的 JSON 会自动要求 AI 补全；平台参数校验会指出具体字段，并明确 `foundationOffsetMm=0` 合法、设备平台顶高必须高于侧平台顶高。

0.7.8 预览版修复结构梁族截面高度读取：除 Revit 标准参数外，现在支持 `d`/`h`/截面高度等常用类型参数，HE240A 的 `d=230` 可直接用于几何高度校验。同时增加“只说即将生成脚本、但未返回执行计划”和空回复的自动修复。

0.7.7 预览版将“直接建模”扩展为全局执行模式：任意对话建模写入在必要查询和内部事务试运行通过后立即提交，不再显示“待执行计划”或要求用户点击执行。可在 `配置 → Agent` 切换回计划审阅模式。

0.7.6 预览版新增设备钢平台“跳过方案预览、直接创建”路由。用户明确分跨数和其他设计条件后，AI 可直接返回建模计划，不再生成 `previewId`；事务试建回滚与正式写入确认仍保留。

0.7.5 预览版修复查询脚本遗漏 `mode` 导致连续校验失败的问题，并让设备钢平台流程复用已读取的结构类型深度，直接进入预览。明确的只读查询可安全补全为 `query`，含糊的脚本仍必须显式声明读写模式。

0.7.4 预览版为 Revit 助手增加独立 BIM+AI 图标，并把醒目的“AI 助手”大按钮放在 Revit 内置 `Add-Ins` 选项卡的“盈碳 AI 助手”面板中；帮助与关于收纳为小按钮。面板初始化异常时不再静默无反应。

0.7.3 预览版修复“口头说正在预览、实际未发命令”的卡点：当模型回复承诺立即查询/预览/执行但返回空计划时，宿主会自动纠正协议并继续取得真实 `previewId`，不再让用户反复点击或催促。

0.7.2 预览版把聊天面板底部状态改为可点击的[处理记录](docs/release-v0.7.2.md)：按时间列出命令、模型、成功/失败、耗时、返回 JSON 和具体异常，并能打开完整日志。批次中的部分失败现在也会计入失败。移除“自动创建设备平台”选项，平台写入统一由待执行计划确认。

0.7.1 预览版新增并实测[对话脚本执行](docs/live-scripting.md)：原生命令不足时主动编译 C# 查询/操作脚本，审阅查询结果后继续规划，写入先试运行回滚再确认提交。修复 Revit 托管包装器变化导致同一文档令牌漂移的问题。不是隔离沙箱，所有脚本运行均强制审阅源码。

0.6.0 预览版新增[轴网定位与设备钢平台](docs/steel-platform.md)：读取当前/链接模型轴网 → 补齐设计条件 → 比较分跨方案 → 真实族试建并回滚检查 → 确认后创建钢柱、梁和三块平台板。

同时保留[对话式设计 Agent](docs/conversational-design.md)的实时模型查询、错误反馈、构件回读与核验，以及选择集编辑、警告查询和矩形房间创建。

当前 EXE 的 Revit 组件针对本机 Revit 2026.5（26.5.0.55 / .NET 10）构建；其他小版本未验证。下方 2027 路径为源码默认配置示例，不代表安装包包含 2027 组件。
