    ...
    public static List<AgentSkill> CreateDefaults()
    {
        ...
        [
            ...
            new AgentSkill
            {
                ...
                Instructions = "识别轴网后先resolve_grid_region和list_structure_types。轴距、交点、已有类型不能反问用户抄录。structureTypes已含梁depthMm、depthParameter和板thicknessMm，d/h等常用族参数已映射，不得再用脚本重复查询截面。只询问模型无法提供的设备宽长、两侧方向、基准标高、荷载、支承、梁高上限、次梁间距。计划审阅模式可用preview_steel_platform比较柱数/跨度；全局直接模式或用户明确跳过预览且bays已确定时，立即返回create_steel_platform_direct完整写入计划。不能声称少柱方案同时具有最小梁高或通过结构验算。",
